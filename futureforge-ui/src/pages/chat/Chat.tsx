...
import { Send, User, Sparkles, Activity, Plus, MessageSquare, Trash2 } from 'lucide-react'
...
export const Chat = () => {
  ...
  const handleSelectSession = async (id: number) => {
    ...
    setIsLoading(true);
    try {
      const response = await api.get(`/api/chat/sessions/${id}/messages`);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If a new session was created, update active ID and refresh sidebar
      if (!activeSessionId && aiMessage.sessionId) {
        setActiveSessionId(aiMessage.sessionId);
        fetchSessions();
      }
    } catch (error: any) {
  ...
  return (
    <div className="relative h-full flex flex-row w-full bg-white/60 backdrop-blur-xl z-10 overflow-hidden">
      
      {/* Sidebar - Chat History */}
      <div className="w-72 border-r border-slate-200 bg-slate-50/50 hidden md:flex flex-col">
        <div className="p-4 border-b border-slate-200 bg-white/80">
          <Button 
            onClick={handleNewChat} 
            className="w-full bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl shadow-[0_0_15px_-5px_rgba(6,182,212,0.4)] flex items-center justify-center gap-2 h-11"
          >
          ...
          <div className="space-y-2">
            ...
            {sessions.map(s => (
              <div 
                ...
                className={`group flex items-center justify-between p-3 cursor-pointer rounded-xl transition-all ${
                  activeSessionId === s.id 
                    ? 'bg-white border border-cyan-200 shadow-sm' 
                    : 'hover:bg-white border border-transparent hover:border-slate-200 text-slate-600 hover:text-slate-900'
                }`}
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <MessageSquare className={`w-4 h-4 flex-shrink-0 ${activeSessionId === s.id ? 'text-cyan-600' : 'text-slate-400'}`} />
                  <div className={`truncate text-sm font-medium ${activeSessionId === s.id ? 'text-cyan-700' : ''}`}>
                    {s.title}
                  </div>
                </div>
                <button 
            ...
            {sessions.length === 0 && (
              <div className="text-sm text-slate-400 text-center py-6 px-4">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white/40">
        ...
        {/* Premium Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 bg-white/80 shrink-0">
        ...
        {/* Input Area */}
        <div className="p-4 sm:p-6 bg-white/80 border-t border-slate-200 backdrop-blur-xl shrink-0">
